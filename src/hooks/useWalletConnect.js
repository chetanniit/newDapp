...
export const useWalletConnect = () => {
  ...
  const signAndSendTransaction = useCallback(async (transaction) => {
    ...
    try {
      ...
      // Try different transaction formats that wallets might expect
      let transactionParam = transaction;
      
      // If it's a TronWeb transaction object, try to format it properly
      if (transaction && typeof transaction === 'object') {
        // Some wallets expect the transaction as-is
      ...
      const result = await signClient.request({
        ...
        request: {
          ...
          params: [transactionParam] // Try array format
        }
      });
      ...
      if (result && result.txid) {
        ...
        // Assume the transaction is already signed and try to broadcast
        const broadcastResult = await tronWeb.trx.sendRawTransaction(result);
        return broadcastResult;
      }
    } catch (error) {
      ...
      if (error.message && error.message.includes('method not supported')) {
        throw new Error('This wallet does not support Tron transactions via WalletConnect. Please use a Tron-compatible wallet like TronLink.');
      }
